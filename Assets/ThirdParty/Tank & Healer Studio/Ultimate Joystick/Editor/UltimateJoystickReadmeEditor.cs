...
public class UltimateJoystickReadmeEditor : Editor
{
	...
	void ShowDocumentation ( DocumentationInfo info )
	{
		...
		var rect = GUILayoutUtility.GetLastRect();
		EditorGUIUtility.AddCursorRect( rect, MouseCursor.Link );
		if( UnityEngine.Event.current.type == EventType.MouseDown && rect.Contains( UnityEngine.Event.current.mousePosition ) && ( info.showMore.faded == 0.0f || info.showMore.faded == 1.0f ) )
		{
